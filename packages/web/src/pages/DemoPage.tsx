...
export function DemoPage() {
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      setMessages(prev => [
        ...
        {
          role: 'agent',
          content: `启动失败: ${error instanceof Error ? error.message : '未知错误'}`,
          timestamp: new Date(),
        },
      ])
    } finally {
  ...
  return (
    <div className="flex flex-col h-full max-w-4xl mx-auto p-4 gap-4">
      <h1 className="text-2xl font-bold">Agent 交互演示</h1>

      {/* Agent 选择 */}
      {!sessionId && (
        <Card className="p-4">
          <h2 className="text-lg font-semibold mb-3">选择 Agent</h2>
      ...
      <Card className="flex-1 p-4 overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto space-y-3">
          {messages.length === 0 ? (
            <div className="text-gray-400 text-center py-8">选择 Agent 并输入问题开始对话</div>
          ) : (
